...
export default function ProdukteTabelle({ kalk, t, handlers }) {
  ...
  return (
    <section className="mb-6">
      <h2 className="text-lg font-bold text-gray-800 mb-3 pb-1 border-b-2 border-blue-500">{t.produkte}</h2>
          ...
          <tbody>
            {zeilen.map(z => (
              <tr key={z.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-2 py-1"><EditableCell value={z.name} onChange={v => updateProduktZeile(z.id, 'name', v)} align="left" placeholder="Producto..." /></td>
                <td className="px-2 py-1"><EditableCell value={z.betrag} onChange={v => updateProduktZeile(z.id, 'betrag', v)} type="number" /></td>
                <td className="px-2 py-1"><EditableCell value={z.menge} onChange={v => updateProduktZeile(z.id, 'menge', v)} type="number" /></td>
                {mehrere && <td className="px-2 py-1"><EditableCell value={z.maseinheit} onChange={v => updateProduktZeile(z.id, 'maseinheit', v)} type="number" /></td>}
                ...
                <td className="px-1">
                  <button onClick={() => removeProduktZeile(z.id)} className="text-red-400 hover:text-red-600 text-xs px-1 py-0.5 rounded hover:bg-red-50">✕</button>
                </td>
              </tr>
            ))}
          </tbody>
      ...
      <button onClick={addProduktZeile} className="mt-2 text-sm text-blue-600 hover:text-blue-800 hover:underline">{t.zeile_hinzufuegen}</button>
