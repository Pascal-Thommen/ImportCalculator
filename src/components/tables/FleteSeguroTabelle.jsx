...
export default function FleteSeguroTabelle({ kalk, t, handlers }) {
  ...
  return (
    <section className="mb-6">
      <h2 className="text-lg font-bold text-gray-800 mb-3 pb-1 border-b-2 border-green-500">{t.flete_seguro}</h2>
          ...
          <tbody>
            {zeilen.map(z => (
              <tr key={z.id} className="border-b border-gray-100 hover:bg-gray-50">
                <td className="px-2 py-1"><EditableCell value={z.beschreibung} onChange={v => updateFleteZeile(z.id, 'beschreibung', v)} align="left" /></td>
                <td className="px-2 py-1"><EditableCell value={z.betrag} onChange={v => updateFleteZeile(z.id, 'betrag', v)} type="number" /></td>
                {mehrere && <td className="px-2 py-1"><DropdownCell value={z.aufteilung} options={aufteilungOptions} onChange={v => updateFleteZeile(z.id, 'aufteilung', v)} /></td>}
                <td className="px-1"><button onClick={() => removeFleteZeile(z.id)} className="text-red-400 hover:text-red-600 text-xs px-1 py-0.5 rounded hover:bg-red-50">✕</button></td>
              </tr>
            ))}
          </tbody>
        </table>
      </div>
      <button onClick={addFleteZeile} className="mt-2 text-sm text-blue-600 hover:text-blue-800 hover:underline">{t.zeile_hinzufuegen}</button>
