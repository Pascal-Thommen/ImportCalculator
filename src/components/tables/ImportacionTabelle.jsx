...
export default function ImportacionTabelle({ kalk, t, handlers }) {
  ...
  return (
    <section className="mb-6">
            ...
            {zeilen.map(z => {
              ...
              return (
                <tr key={z.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-2 py-1"><EditableCell value={z.beschreibung} onChange={v => updateImportacionZeile(z.id, 'beschreibung', v)} align="left" /></td>
                  <td className="px-2 py-1"><EditableCell value={z.betrag} onChange={v => updateImportacionZeile(z.id, 'betrag', v)} type="number" /></td>
                  {mehrere && <td className="px-2 py-1"><DropdownCell value={z.aufteilung} options={aufteilungOptions} onChange={v => updateImportacionZeile(z.id, 'aufteilung', v)} /></td>}
                  <td className="px-2 py-1"><DropdownCell value={z.impuesto} options={impuestoOptions} onChange={v => updateImportacionZeile(z.id, 'impuesto', v)} /></td>
                  ...
                  <td className="px-1"><button onClick={() => removeImportacionZeile(z.id)} className="text-red-400 hover:text-red-600 text-xs px-1 py-0.5 rounded hover:bg-red-50">✕</button></td>
                </tr>
              )
            })}
          </tbody>
      ...
      <button onClick={addImportacionZeile} className="mt-2 text-sm text-blue-600 hover:text-blue-800 hover:underline">{t.zeile_hinzufuegen}</button>
