...
export default function NationaleKostenTabelle({ kalk, t, handlers }) {
            ...
            {zeilen.map(z => {
              ...
              return (
                <tr key={z.id} className="border-b border-gray-100 hover:bg-gray-50">
                  ...
                  <td className="px-2 py-1">
                    <EditableCell value={z.betrag} onChange={v => updateNationaleZeile(z.id, 'betrag', v)} type="number" />
                  </td>
                  {mehrere && (
                    <td className="px-2 py-1">
                      <DropdownCell value={z.aufteilung} options={aufteilungOptions} onChange={v => updateNationaleZeile(z.id, 'aufteilung', v)} />
                    </td>
                  )}
                  <td className="px-2 py-1">
                    <DropdownCell value={z.impuesto} options={impuestoOptions} onChange={v => updateNationaleZeile(z.id, 'impuesto', v)} />
                  </td>
                  ...
                  <td className="px-1">
                    <button
                      onClick={() => removeNationaleZeile(z.id)}
